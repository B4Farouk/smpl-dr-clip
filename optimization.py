    ...
    def optimize(self, pose, shape, n_passes=1000, tracker_settings=None):
        ...
        for n in range(1, n_passes+1):
            ...
            if track_pose and n % pose_interleaving == 0:
                intermediate_poses.loc[len(intermediate_poses)] = {"pass": n, "pose": pose.cpu().detach().numpy()}
            # shape tracking
            if track_shape and n % shape_interleaving == 0:
                intermediate_shapes.loc[len(intermediate_shapes)] = {"pass": n, "pose": shape.cpu().detach().numpy()}
